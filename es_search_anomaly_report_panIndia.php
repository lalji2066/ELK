...
if(isset($_GET['serach_term']))
$serach_term = $_GET['serach_term'];
else
$serach_term = $argv[2];
#echo $source;exit;
if(!empty($source)){
...
############################################################################################################################################
		...
		################################################################COMPANYNAME SERACH END#######################################################
		
		if($serach_term == 'ADDRESS'){
############################################################################################################################################
							###ADDRESS####
					unset($source_search_params['body']['query']['match']);
					unset($dest_search_params['body']['query']['match']);
############################################################################################################################################
			...
			if(!empty($source_search_addr_response['hits']['hits']) && count($source_search_addr_response['hits']['hits'])>0){
			...
			#	if(trim($data['_source']['parentid']) != trim($src_add_result['_source']['parentid'])){
				...
				if(!empty($new_address)){
					...
					if(!empty($dest_search_addr_response['hits']['hits']) && count($dest_search_addr_response['hits']['hits'])>0){
...
############################################################################################################################################
			...
			if(!empty($source_search_phone_response['hits']['hits']) && count($source_search_phone_response['hits']['hits'])>0){
			...
			if(!empty($dest_search_phone_response['hits']['hits']) && count($dest_search_phone_response['hits']['hits'])>0){
